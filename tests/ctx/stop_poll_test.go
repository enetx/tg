...
func TestStopPoll_Business(t *testing.T) {
	...
	if ctx.StopPoll().Business(g.String("biz_123")) == nil { t.Error("Business should return builder") }
}
...
func TestStopPoll_Markup(t *testing.T) {
	...
	if ctx.StopPoll().Markup(keyboard.Inline().Button(btn1)) == nil { t.Error("Markup should return builder") }
}

func TestStopPoll_Timeout(t *testing.T) {
	bot := &mockBot{}
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, EffectiveMessage: &gotgbot.Message{MessageId: 789, Poll: &gotgbot.Poll{Id: "poll123"}}, Update: &gotgbot.Update{UpdateId: 1}})
	if ctx.StopPoll().Timeout(time.Minute) == nil { t.Error("Timeout should return builder") }
}

func TestStopPoll_APIURL(t *testing.T) {
	bot := &mockBot{}
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, EffectiveMessage: &gotgbot.Message{MessageId: 789, Poll: &gotgbot.Poll{Id: "poll123"}}, Update: &gotgbot.Update{UpdateId: 1}})
	if ctx.StopPoll().APIURL(g.String("https://api.example.com")) == nil { t.Error("APIURL should return builder") }
}
