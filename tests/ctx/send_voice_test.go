...
func TestContext_SendVoiceChaining(t *testing.T) {
	...
	result = ctx.SendVoice(filename).AllowPaidBroadcast()
	if result == nil {
		t.Error("AllowPaidBroadcast method should return SendVoice for chaining")
	}

	// Test Effect method
	result = ctx.SendVoice(filename).Effect(effects.Fire)
	if result == nil {
		t.Error("Effect method should return SendVoice for chaining")
	}
}
...
func TestSendVoice_CaptionEntities(t *testing.T) {
	...
	if ctx.SendVoice(filename).CaptionEntities(ent) == nil {
...
func TestSendVoice_After(t *testing.T) {
	...
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, Update: &gotgbot.Update{UpdateId: 1}})
	filename := g.String("voice.ogg")
	if ctx.SendVoice(filename).After(time.Minute) == nil {
...
func TestSendVoice_DeleteAfter(t *testing.T) {
	...
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, Update: &gotgbot.Update{UpdateId: 1}})
	filename := g.String("voice.ogg")
	if ctx.SendVoice(filename).DeleteAfter(time.Hour) == nil {
...
func TestSendVoice_Markdown(t *testing.T) {
	...
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, Update: &gotgbot.Update{UpdateId: 1}})
	filename := g.String("voice.ogg")
	if ctx.SendVoice(filename).Markdown() == nil {
...
func TestSendVoice_Markup(t *testing.T) {
	...
	if ctx.SendVoice(filename).Markup(keyboard.Inline().Button(btn1)) == nil {
...
func TestSendVoice_ReplyTo(t *testing.T) {
	...
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, Update: &gotgbot.Update{UpdateId: 1}})
	filename := g.String("voice.ogg")
	if ctx.SendVoice(filename).Reply(reply.New(123)) == nil {
...
func TestSendVoice_Business(t *testing.T) {
	...
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, Update: &gotgbot.Update{UpdateId: 1}})
	filename := g.String("voice.ogg")
	if ctx.SendVoice(filename).Business(g.String("biz_123")) == nil {
...
func TestSendVoice_Thread(t *testing.T) {
	...
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, Update: &gotgbot.Update{UpdateId: 1}})
	filename := g.String("voice.ogg")
	if ctx.SendVoice(filename).Thread(456) == nil {
...
func TestSendVoice_APIURLWithExistingRequestOpts(t *testing.T) {
	bot := &mockBot{}
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, Update: &gotgbot.Update{UpdateId: 1}})
	filename := g.String("test.ogg")
...
func TestSendVoice_FileClosing(t *testing.T) {
	bot := &mockBot{}
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, Update: &gotgbot.Update{UpdateId: 1}})

	// Create a temporary file
	tempFile := "/tmp/test_voice.ogg"
	os.WriteFile(tempFile, []byte("test voice content"), 0644)
	defer os.Remove(tempFile)

	sendResult := ctx.SendVoice(g.String(tempFile)).Send()

	// This will fail with mock bot, but covers the file closing path
	if sendResult.IsErr() {
		t.Logf("SendVoice Send with file closing failed as expected: %v", sendResult.Err())
	}
}

func TestSendVoice_TimersIntegration(t *testing.T) {
	bot := &mockBot{}
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, Update: &gotgbot.Update{UpdateId: 1}})
	...
	if result == nil {
		t.Error("Voice with timers should return builder")
	}
	...
	if sendResult.IsErr() {
		t.Logf("SendVoice Send with timers failed as expected: %v", sendResult.Err())
	}
}

func TestSendVoice_DirectMessagesTopic(t *testing.T) {
	bot := &mockBot{}
	rawCtx := &ext.Context{
		EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"},
		Update:        &gotgbot.Update{UpdateId: 1},
	...
	for _, topicID := range topicIDs {
		result := ctx.SendVoice(filename).DirectMessagesTopic(topicID)
		if result == nil {
			t.Errorf("DirectMessagesTopic method should return SendVoice builder for chaining with topicID: %d", topicID)
		}
...
func TestSendVoice_SuggestedPost(t *testing.T) {
	bot := &mockBot{}
	rawCtx := &ext.Context{
		EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"},
		Update:        &gotgbot.Update{UpdateId: 1},
	...
	result := ctx.SendVoice(filename).SuggestedPost(nil)
	if result == nil {
		t.Error("SuggestedPost method should return SendVoice builder for chaining with nil params")
	}
...
func TestSendVoice_SendWithAllOptions(t *testing.T) {
	bot := &mockBot{}
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, Update: &gotgbot.Update{UpdateId: 1}})

	filename := g.String("voice.ogg")
	btn1 := keyboard.NewButton().Text(g.String("Play Again")).Callback(g.String("play_again"))
	ent := entities.New(g.String("Voice message")).Bold(g.String("Voice"))

	// Test with all options to cover different code paths
	sendResult := ctx.SendVoice(filename).
		Caption(g.String("Test <b>voice</b> message")).
		HTML().
		CaptionEntities(ent).
		Duration(45).
	...
	if sendResult.IsErr() {
		t.Logf("SendVoice Send with all options failed as expected: %v", sendResult.Err())
	}
}
