...
func TestSetStickerKeywords_Timeout(t *testing.T) {
	...
	sticker := g.String("sticker_id_123")
	if ctx.SetStickerKeywords(file.Input(sticker).UnwrapOrDefault()).Timeout(time.Minute) == nil { t.Error("Timeout should return builder") }
}
...
func TestSetStickerKeywords_APIURL(t *testing.T) {
	...
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 123, Type: "private"}, Update: &gotgbot.Update{UpdateId: 1}})
	sticker := g.String("sticker_id_123")
	if ctx.SetStickerKeywords(file.Input(sticker).UnwrapOrDefault()).APIURL(g.String("https://api.example.com")) == nil { t.Error("APIURL should return builder") }
}
