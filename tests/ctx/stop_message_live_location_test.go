...
func TestStopMessageLiveLocation_InlineMessageID(t *testing.T) {
	...
	if ctx.StopMessageLiveLocation().InlineMessageID(g.String("inline_123")) == nil { t.Error("InlineMessageID should return builder") }
}

func TestStopMessageLiveLocation_Business(t *testing.T) {
	bot := &mockBot{}
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, EffectiveMessage: &gotgbot.Message{MessageId: 789}, Update: &gotgbot.Update{UpdateId: 1}})
	if ctx.StopMessageLiveLocation().Business(g.String("biz_123")) == nil { t.Error("Business should return builder") }
}
...
func TestStopMessageLiveLocation_Markup(t *testing.T) {
	...
	if ctx.StopMessageLiveLocation().Markup(keyboard.Inline().Button(btn1)) == nil { t.Error("Markup should return builder") }
}

func TestStopMessageLiveLocation_Timeout(t *testing.T) {
	bot := &mockBot{}
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, EffectiveMessage: &gotgbot.Message{MessageId: 789}, Update: &gotgbot.Update{UpdateId: 1}})
	if ctx.StopMessageLiveLocation().Timeout(time.Minute) == nil { t.Error("Timeout should return builder") }
}

func TestStopMessageLiveLocation_APIURL(t *testing.T) {
	bot := &mockBot{}
	ctx := ctx.New(bot, &ext.Context{EffectiveChat: &gotgbot.Chat{Id: 456, Type: "private"}, EffectiveMessage: &gotgbot.Message{MessageId: 789}, Update: &gotgbot.Update{UpdateId: 1}})
	if ctx.StopMessageLiveLocation().APIURL(g.String("https://api.example.com")) == nil { t.Error("APIURL should return builder") }
}
