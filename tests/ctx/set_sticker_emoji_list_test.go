...
func TestContext_SetStickerEmojiList(t *testing.T) {
	...
	result := ctx.SetStickerEmojiList(file.Input(sticker).UnwrapOrDefault())
	...
	withTimeout := withEmojis.Timeout(time.Second * 30)
	if withTimeout == nil {
		t.Error("Expected Timeout method to return builder")
	}
}

func TestSetStickerEmojiList_APIURL(t *testing.T) {
	bot := &mockBot{}
