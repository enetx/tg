...
type CallbackHandlers struct{ bot *Bot }

func (h *CallbackHandlers) Any(fn Handler) *CallbackHandler {
	return h.bot.handleCallback(nil, fn)
}

func (h *CallbackHandlers) Equal(data String, fn Handler) *CallbackHandler {
	return h.bot.handleCallback(func(q *gotgbot.CallbackQuery) bool {
