...
func main() {
	...
	b.Command("create_invite", func(ctx *ctx.Context) error {
		...
		if result.IsErr() {
			return ctx.Reply(g.Format("Failed to create invite link: {}", result.Err())).Send().Err()
		}

		inviteLink := result.Ok()
		return ctx.Reply("Created invite link: " + g.String(inviteLink.InviteLink)).Send().Err()
	})
	...
	b.Command("edit_invite", func(ctx *ctx.Context) error {
		...
		inviteLink := g.String("https://t.me/+example")
		...
		if result.IsErr() {
			return ctx.Reply(g.Format("Failed to edit invite link: {}", result.Err())).Send().Err()
		}

		editedLink := result.Ok()
		return ctx.Reply("Edited invite link: " + g.String(editedLink.InviteLink)).Send().Err()
	})
	...
	b.Command("revoke_invite", func(ctx *ctx.Context) error {
		...
		if result.IsErr() {
			return ctx.Reply(g.Format("Failed to revoke invite link: {}", result.Err())).Send().Err()
		}

		revokedLink := result.Ok()
		return ctx.Reply("Revoked invite link: " + g.String(revokedLink.InviteLink)).Send().Err()
	})
	...
	b.On.ChatJoinRequest.Any(func(ctx *ctx.Context) error {
		...
		if result.IsErr() {
			g.Println("Failed to approve join request from {}:{}", user.FirstName, result.Err())
			return result.Err()
		}

		g.Println("Approved join request from {1.FirstName} {1.LastName}", user)
		return nil
	})
	...
	b.Command("decline_request", func(ctx *ctx.Context) error {
		...
		if result.IsErr() {
			return ctx.Reply(g.Format("Failed to decline join request: {}", result.Err())).Send().Err()
		}
