...
func (f *Forward) Timeout(duration time.Duration) *Forward {
	if f.opts.RequestOpts == nil {
		f.opts.RequestOpts = new(gotgbot.RequestOpts)
	}

	f.opts.RequestOpts.Timeout = duration

	return f
}

// APIURL sets a custom API URL for this request.
func (f *Forward) APIURL(url String) *Forward {
	if f.opts.RequestOpts == nil {
		f.opts.RequestOpts = new(gotgbot.RequestOpts)
	}

	f.opts.RequestOpts.APIURL = url.Std()
